<template>
    <section
        class="w-full min-h-svh px-6 py-6 flex flex-col items-center justify-center text-lg lg:text-2xl text-center gap-y-4">
        <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 w-full">
            <div class="w-full order-1 lg:order-2 flex justify-center items-center">
                <svg xmlns="http://www.w3.org/2000/svg" class="w-full" viewBox="0 0 842.72998 573"
                    ...
                    <path
                        ...
                        transform="translate(-178.63501 -163.5)" fill="#f2f2f2" />
                    <path
                        ...
                        transform="translate(-178.63501 -163.5)" fill="#fff" />
                    <path
                        ...
                        transform="translate(-178.63501 -163.5)" fill="#ccc" />
                    <circle cx="36.81607" cy="29.45637" r="13.13371" fill="#ccc" />
                    <path
                    ...
                    <path
                        ...
                        transform="translate(-178.63501 -163.5)" fill="#3f3d56" />
                    <circle cx="36.81607" cy="363.05254" r="13.13371" fill="#ccc" />
                    <path
                    ...
                    <path
                        ...
                        transform="translate(-178.63501 -163.5)" fill="#ccc" />
                    <circle cx="239.07517" cy="188.81202" r="13.13371" fill="#3f3d56" />
                    <path
                    ...
                    <path
                        ...
                        transform="translate(-178.63501 -163.5)" fill="#ffb8b8" />
                    <circle cx="670.29019" cy="209.30068" r="24.56103" fill="#ffb8b8" />
                    <polygon points="641.436 547.373 629.177 547.372 623.344 500.084 641.439 500.085 641.436 547.373"
                        fill="#ffb8b8" />
                    <path
                        d="M823.198,722.75707l-39.53052-.00146v-.5a15.38605,15.38605,0,0,1,15.38648-15.38623h.001l24.1438.001Z"
                        transform="translate(-178.63501 -163.5)" fill="#2f2e41" />
                    <polygon points="678.436 560.373 666.177 560.372 664.479 514.214 678.439 513.085 678.436 560.373"
                        fill="#ffb8b8" />
                    <path
            ...
            <div class="w-full order-2 lg:order-1">
                <div class="border-2 border-violet-500 w-full rounded-xl px-2 py-4 flex flex-col gap-y-4">
                    <input v-model="subject" type="text"
                        class="outline-0 w-full bg-neutral-900/10 backdrop-blur-xs rounded-xl px-4 py-2"
                        :placeholder="$t('subject')">
                    <textarea v-model="message"
                        class="min-h-72 outline-0 w-full bg-neutral-900/10 backdrop-blur-xs rounded-xl px-4 py-2"
                        :placeholder="$t('message')"></textarea>
                    <button @click="sendToWhatsApp"
                        class="outline-0 w-full bg-neutral-900/10 backdrop-blur-xs rounded-xl px-4 py-2 cursor-pointer transition-colors duration-300 hover:bg-violet-600 hover:text-white">{{
                            $t('send') }}</button>
                    <NuxtLink to="/"
                        class="outline-0 w-full bg-neutral-900/10 backdrop-blur-xs rounded-xl px-4 py-2 cursor-pointer transition-colors duration-300 hover:bg-violet-600 hover:text-white">
                        {{ $t('back') }}</NuxtLink>
...
<script setup>
const phoneNumber = '989023131107'
const subject = ref('')
const message = ref('')

const sendToWhatsApp = () => {
    const fullMessage = `موضوع: ${subject.value}\n${message.value}`
    const encodedMessage = encodeURIComponent(fullMessage)
    const whatsappURL = `https://wa.me/${phoneNumber}?text=${encodedMessage}`
    window.open(whatsappURL, '_blank')
}
useHead({
    title: $t('name') + ' - ' + $t('contact')
})
</script>
